...
function generateStreamFromCallback(
  callback: (push: (data: any) => void, close: () => void) => void,
) {
  const stream = new ReadableStream({
    async start(controller) {
      // Call the callback function with a "push" function as parameter
      callback(
        (data) => {
          // Push each chunk of data into the readable stream
...
export const LangchainStream = async (
  model: OpenAIModel,
  systemPrompt: string,
  temperature: number,
  key: string,
  ...
  const stream = generateStreamFromCallback(async (push, close) => {
    ...
    await chat.call(messageHistory);
  });

  return stream;
};

// class IdealoWebBrowser extends WebBrowser {
//   name = 'idealo-web-browser';
//   description =
//     'useful for when you need to find information and pricing data of any kind of product. ';
// }

// export const run = async (prompt: string, key: string) => {
//   const model = new OpenAI({ temperature: 0, openAIApiKey: key });
//   const embeddings = new OpenAIEmbeddings({ openAIApiKey: key });
//   const tools = [new Calculator(), new WebBrowser({ model, embeddings })];
...
//   console.log(`Executing with input "${prompt}"...`);

//   const result = await executor.call({ prompt });
//   /*
